...
```text
DIY-EAF-ASCOM-Setup-1.26.0.exe
```
...
Manual Control mặc định mở ở mức `Slowest` để tránh motor bị stall hoặc chạy quá tay. Chỉ tăng lên `Normal`, `Fast` hoặc `Fastest` khi đã chắc chắn motor quay ổn dưới tải.
